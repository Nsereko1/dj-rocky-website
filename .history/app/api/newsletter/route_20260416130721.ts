...
export async function POST(request: NextRequest) {
  try {
    ...
    // 1. Add the contact to Resend Audience
    ...
    await resend.contacts.create({
      ...
      email: email,
      // unsubscribed: false, // optional, defaults to false
    });
    ...
    // 3. Notify admin (without fake total count)
    await resend.emails.send({
      from: FROM_EMAIL,
      to: 'djrockyug@gmail.com',
      subject: `📧 New Newsletter Subscriber: ${email}`,
      text: `New subscriber: ${email}\nCheck your Resend Audience for the full list.`,
    });
    ...
    // Handle duplicate contact error (already in audience)
    if (error?.name === 'ResendError' && error?.message?.includes('already exists')) {
      return NextResponse.json(

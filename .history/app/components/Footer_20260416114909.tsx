...
export default function Footer() {
  ...
  return (
    <footer className="bg-gray-100 dark:bg-black/50 text-gray-600 dark:text-white/70 py-4 sm:py-5 text-xs sm:text-sm border-t border-gray-200 dark:border-white/5">
      <div className="container mx-auto px-4 sm:px-6">
        <div className="flex flex-col gap-3">
          {/* Newsletter – three columns: left text, middle form, right description */}
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            {/* Middle: input + button (input width reduced by 20%) */}
            <div className="sm:w-2/4 flex justify-center">
              <form onSubmit={handleSubmit} className="flex items-center gap-2 w-full max-w-md">
                <input
                  ...
                  required
                  className="w-4/5 px-3 py-1.5 rounded-full bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-sm focus:outline-none focus:ring-1 focus:ring-purple-500"
                />
                <button
                  type="submit"
                  disabled={status === "loading"}
                  className="w-1/5 px-3 py-1.5 bg-purple-600 hover:bg-purple-700 text-white font-medium rounded-full text-sm transition disabled:opacity-50 whitespace-nowrap"
                >

    ...
    <footer className="bg-gray-100 dark:bg-black/50 text-gray-600 dark:text-white/70 py-4 sm:py-5 text-xs sm:text-sm border-t border-gray-200 dark:border-white/5">
      <div className="container mx-auto px-4 sm:px-6">
        <div className="flex flex-col gap-3">
          ...
          {/* Newsletter – single horizontal line on desktop */}
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 flex-wrap">
            <span className="font-bold text-gray-800 dark:text-white whitespace-nowrap">
              🎵 Join the Future Afro Movement
            </span>

            <form onSubmit={handleSubmit} className="flex flex-col xs:flex-row gap-2 items-center sm:flex-1 sm:justify-end">
              <input
                type="email"
                value={email}
                ...
                required
                className="px-3 py-1.5 rounded-full bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-sm focus:outline-none focus:ring-1 focus:ring-purple-500 w-full xs:w-auto"
              />
              <button
                type="submit"
      ...
      {/* Status message inline (optional) */}
      {status === "success" && (
